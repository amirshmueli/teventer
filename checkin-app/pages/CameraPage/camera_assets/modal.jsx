...
import { get_token, scan_ticket } from "../../../Server";
...
const PopUp = ({ visible, setVisible, setScan, urlData, Tinterval }) => {
  ...
  const { user, tickets, event, token} = useSelector(state => { console.log("changed"); return state});
  
  // icons
  const checkMark = <Ionicons name="checkmark-circle" color="#00c907" size={150} />;
  ...
  const LoadingIndicator = <View style={{
    ...
    <ActivityIndicator size="large" color="#806739" style={{
      height: 150
    }}/>
  </View>
  ...
  const handleUrl = async () => {
    console.log(urlData);
    if (!urlData.includes(event.eventId)) { handleResult(null, "URL error"); return;};
    setIsLoading(true);
    setIcon(LoadingIndicator);
    let [data, error] = await scan_ticket(user.username, token, urlData);
  ...
  const handleResult = (data, error) => {
    ...
    if (status == "300") {
      ...
      setTicketData({
        ...
        owner: resp.message.guest.slice(1,-1),
        type: JSON.parse(resp.message.res).tickets[0].name,
      })
      console.log(Tinterval);
      if (Tinterval == 0) { return };
  ...
  const render_messages = () => {
    return (
      isLoading ?
        <Text style={[styles.loadingText, { fontFamily: "brandon" }]}>Authenticating Ticket</Text> :
        <View>
          {
            TicketData.message === undefined ?
              null :
              <Text style
                ={styles.mainMessage}>{TicketData.message}</Text>
          }
          {
        ...
        <View style={styles.centeredView}>
            ...
            {render_messages()}
          {
            ...
            <TouchableOpacity
                ...
                onPress={() => { setVisible(false); setScan(false)}}
            >
